...
- 画像トリミング＋位置調整UI（280x280キャンバス、ドラッグ移動、ズーム50%〜200%）
...
### UX
- 画像位置調整エディタ（アップロード直後に表示、確定/スキップ選択可能）
- おすすめパターン8種（ワンクリック適用）
...
│   ├── ImageAdjustEditor.tsx  # 画像位置調整（ドラッグ + ズーム、280x280キャンバス）
...
```
画像アップロード
  → 位置・ズーム調整（ImageAdjustEditor、280x280キャンバス）
    → AI背景透過（or スキップ）
      → 224px高解像度キャンバスに中心配置
...
### 最優先事項
**ユーザーフィードバックの収集。** 機能はMVP+UX改善+サブスク限定機能+Discord対応+画像位置調整まで完了。実際に使った人の声を聞いてから次の判断をするのが最も効率的。
