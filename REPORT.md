...
- 透過後ブラシ微調整エディタ（消しゴム/復元ブラシ、undo最大20回、レスポンシブキャンバス）
...
- 限定アニメーション5種[限定]（ゲーミング / グリッチ / キラキラ / 残像 / 高速回転）
...
- 透過後ブラシ微調整（消しゴムで不要部分を透明化、復元ブラシで元画像から復元、ブラシサイズ5〜60px）
...
│   ├── SettingsPanel.tsx      # 設定UI（フチ取り/テキスト/アニメーション + 限定ロックUI）
...
├── hooks/
│   └── useEmoteProcessor.ts   # 処理パイプライン統合フック（ExportMode対応、ブラシ編集ステージ管理）
├── lib/
│   ├── backgroundRemoval.ts   # @imgly/background-removal ラッパー（標準/高精度モード対応）
│   ├── canvasPipeline.ts      # Canvas描画パイプライン（中心配置/フチ取り/テキスト/縮小）
│   ├── gifEncoder.ts          # gif.js + Web Worker アニメーション生成（13種）
│   ├── visibilityChecker.ts   # 28px視認性チェック
│   └── zipExporter.ts         # JSZip ZIP書き出し（動的ファイル名対応）
└── types/
    └── emote.ts               # 型定義 + 定数（Twitch/Discordサイズ/フォント/プリセット等）
```
...
| VTuber・イラスト素材の透過精度が低い | smallモデルでは複雑なイラストに対応しきれない | 標準/高精度モード切替（small/mediumモデル）+ ブラシ微調整エディタ追加 |
...
### 最優先事項
**ユーザーフィードバックの収集。** 機能はMVP+UX改善+サブスク限定機能+Discord対応+トリミング＋位置調整+背景透過精度改善+ブラシ微調整まで完了。実際に使った人の声を聞いてから次の判断をするのが最も効率的。
