...
| コード規模 | 24ファイル / 約4,874行（src/配下） |
| コミット数 | 48 |
| 最新コミット | `25dcb7f` feat: アニメーション速度調整 + 限定アニメーション9種追加 |
...
- ブラウン管 / 地震 / パーティ / ひっくり返る / 幽霊 / デジタル崩壊 / スパイラル / 鼓動 / バネ
...
- 未認証時はロック項目がグレーアウト表示
- ログアウト時にサブスク限定configを自動リセット（24種全アニメーション対応）
...
│   ├── gifEncoder.ts            # gif.js + Web Worker アニメーション生成（31種、速度可変）
...
### 最優先事項
**ユーザーフィードバックの収集。** 機能はMVP+UX改善+サブスク限定機能+Discord対応+トリミング＋位置調整+背景透過精度改善+ブラシ微調整+セキュリティ強化+アニメーション31種+速度調整+モバイルミニプレビューまで完了。実際に使った人の声を聞いてから次の判断をするのが最も効率的。
