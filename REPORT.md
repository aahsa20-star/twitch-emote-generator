...
| コード規模 | 27ファイル / 約5,502行（src/配下） |
| コミット数 | 61 |
| 最新コミット | `0ace013` copy: 合言葉UIにDiscordサーバー案内リンクを追加 |
...
- ログアウト時にサブスク限定configを自動リセット（25種アニメーション+フレーム+合成+サブ画像サイズ）
...
│   ├── SettingsPanel.tsx        # 設定UI（フチ取り/フレーム/合成/テキスト/アニメーション/速度 + 限定ロックUI）
...
│   ├── canvasPipeline.ts        # Canvas描画パイプライン（中心配置/合成/フチ取り/フレーム/テキスト/縮小）
...
### 最優先事項
**ユーザーフィードバックの収集。** 機能はMVP+UX改善+サブスク限定機能+Discord対応+トリミング＋位置調整+背景透過精度改善+ブラシ微調整+セキュリティ強化+アニメーション32種+速度調整+モバイルミニプレビュー+カラーピッカー改善+シェアモーダル+アナリティクス+フレーム7種+2画像合成+Discord案内リンクまで完了。実際に使った人の声を聞いてから次の判断をするのが最も効率的。
