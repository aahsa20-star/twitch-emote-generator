...
| テスト | vitest（`npm test`） |
| 最新の作業 | 2026-09-06 UI/UX 刷新を本番公開（main b85f469 / Vercel dpl_6ZEJNBnf1HpwHMT1KTTZfYB6HHVC）。下の「コア機能」は新しい順の変更履歴 |
...
### コア機能
- **UI/UX 刷新の本番公開（2026-09-06）** — PR #4（`feat/uiux-refresh` = d65830c）を通常 merge し main を b85f469 に更新。Vercel が本番デプロイ `dpl_6ZEJNBnf1HpwHMT1KTTZfYB6HHVC`（`twitch-emote-generator-ky18vx4f4-…`、Ready、`https://twitch-emote-generator.vercel.app` に反映）を作成。環境変数・Supabase・Twitch OAuth 設定は変更していない。公開前の確認は Vercel Preview（dpl_4RDsdtrpTavautfmoNFhYfrNsz5z、共有リンク）で行い、Aki の確認後に公開。**復旧先の記録**: 直前の本番 `dpl_6SeuTrj1Z3YSuUGeLScVnC4pnXiq`（d9706df、旧 UI）。**公開直後の確認（実施済み）**: 新規状態で `/` が新しい入口（2 カラム、フォロー / 合言葉）を表示、誤合言葉 401、旧 Cookie `emote-subscriber=1` は `locked` と保存 403、`/dev/animations` `/dev/gif-check` と廃止 API は 404、`/privacy` 200。合言葉 Cookie（当日発行）で解放 → 工程 1「まずは、主役を選ぼう。」→ サンプル → この範囲で使う → ハートぷかぷか → 拡大見本が GIF（blob）、Worker チャンク読み込みあり → 保存へ進む → 「112px GIF を保存」→ 完了カード → `~/Downloads/emote_112px (7).gif`（112×112、20 フレーム、50 ms、透明あり）。コンソールエラーなし。**未確認（ユーザー引き継ぎ）**: Twitch 実ログイン → フォロー解放、実機 iPhone の 2 段階保存・順次保存・キーボード表示中の dock。
- **追加レビュー 13 対応（feat/uiux-refresh、2026-09-06、ローカル・レビュー待ち・未 push）** — **§1 保存待機中の同一目的地での設定変更**: 再現 — 保存キーが `outputGen` 由来だったため、同じ目的地・形式で文字や動きを変えても次の出力が成功するまでキーが変わらず、開始済みの保存クロージャが API 応答後の比較を通過していた（`requestedGen 6 / outputGen 5` でも同一キー）。修正 — `ExportPanel` は `requestedGen` を受け取り `savePlanKey(plan, requestedGen, files)` で束ねる（変更した瞬間に準備済み・案内文・完了カードが解除、再生成の完了を待たない）。`lib/ui/save-flow.ts` の `runGuardedSave` が保存の非同期処理を統一し、権限 API 応答後と ZIP 生成後の両方で「開始時の要求が今も同じ」かつ「出力が current」を再確認して不一致なら中止（`SaveActions` は `latestKey` / `latestCondition` の ref で判定）。iOS の 2 タップ目も同じ判定を通す。テスト（`save-flow.test.ts`）は親が渡す世代（`outputCondition` / `savePlanKey`）と非同期保存処理を結合したモデルで、①権限保留 → 同一目的地で文字変更 → 描画保留 → 権限 OK → 中止・未配信、②ZIP 生成中の変更 → 中止、③描画失敗 → 中止、④変更なしは配信・拒否は未配信、⑤準備済みキーは要求変更の時点で失効、を検証。ブラウザ（Chrome）— download-check を保留 → 同じ目的地で文字を変更（この時点で「更新中」）→ 保留を解除 → 「設定または出力が変わったため中止しました」、アンカークリック 0 回・`window.open` 0 回。アプリ内ブラウザのモバイル emulation — 準備する → 開く の後に文字を変更すると直後に「準備する」に戻り古い案内文は消える。**§2 GIF 検証中の選択競合**: 再現 — `gifCheckRef` は GIF を選んだときだけ進むため、GIF の読み込み中に静止画・動画・サンプルを選んでも先の GIF が後から select/confirm して工程 3 へ移動した。修正 — `createSelectionSequence`（`lib/ui/save-flow.ts`）の共通の選択番号を素材の種類によらず新しい選択と「選び直しをやめる」で進め、GIF の検証完了時に番号が最新のときだけ採用。テストは GIF A の検証保留 → 静止画 / 動画 / サンプル / GIF B を選択 → A 完了 で B の候補（または確定）が維持されること、キャンセルでも失効することを検証。ブラウザ（Chrome）— `File.prototype.arrayBuffer` を GIF だけ保留にして GIF A → 静止画 B（工程 2、B の調整画面）→ A の保留解除 → 工程 2 と B の候補のまま、GIF の採用なし。GIF A → GIF B → 両方解除 → 確定は b.gif。**結果**: tsc 0、vitest 23 ファイル 177 件（新規 9）、ESLint 6 error / 13 warning（error は未変更ファイルの既存指摘のみ）、`next build` 成功。**未確認（端末・アカウントが必要）**: 実機 iPhone、実 Twitch ログイン。ZIP 生成中の変更はブラウザでは生成が速すぎて保留できないためテストで検証。
