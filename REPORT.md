...
| コミット数 | 65+ |
| 最新コミット | feat: テキスト・サブ画像のドラッグ直接配置 |
...
- テキスト・サブ画像のドラッグ直接配置（DragPositionCanvasコンポーネント、224px内部解像度、マウス+タッチ対応）
  - テキスト: 当たり判定+紫破線インジケーター、ドラッグでtextOffsetX/Y連動
  - サブ画像: 当たり判定+緑破線インジケーター、ドラッグでsubImageOffsetX/Y連動
  - スライダーとの双方向同期（スライダー変更→キャンバス更新、ドラッグ→スライダー更新）
  - サブ画像横位置・縦位置スライダー（-50〜50px、overlay時のみ表示）
...
### 最優先事項
**ユーザーフィードバックの収集。** 機能はMVP+UX改善+サブスク限定機能+Discord対応+トリミング＋位置調整+背景透過精度改善+ブラシ微調整+セキュリティ強化+アニメーション32種+速度調整+モバイルミニプレビュー+カラーピッカー改善+シェアモーダル+アナリティクス+フレーム7種+2画像合成+Discord案内リンク+サブスクバッジ作成+Canvasメモリ最適化+ドラッグ直接配置まで完了。実際に使った人の声を聞いてから次の判断をするのが最も効率的。
