...
- AI背景透過 → 位置調整 → フチ取り → テキスト → アニメーション → 3サイズ同時出力
- 技術知識不要、初見で使える
...
- 画像トリミング＋位置調整UI（280x280キャンバス、8ハンドル矩形選択、ドラッグ移動、ズーム50%〜200%）
...
### UX
- 画像位置調整エディタ（アップロード直後に表示、8ハンドルトリミング+ドラッグ移動+ズーム、確定/スキップ選択可能）
- おすすめパターン8種（ワンクリック適用）
...
│   ├── ImageAdjustEditor.tsx  # 画像位置調整（8ハンドルトリミング + ドラッグ + ズーム、280x280キャンバス）
...
├── hooks/
│   └── useEmoteProcessor.ts   # 処理パイプライン統合フック（ExportMode対応）
├── lib/
...
```
画像アップロード
  → トリミング＋位置・ズーム調整（ImageAdjustEditor、8ハンドル矩形選択、280x280キャンバス）
    → AI背景透過（or スキップ）
      → 224px高解像度キャンバスに中心配置
...
### 最優先事項
**ユーザーフィードバックの収集。** 機能はMVP+UX改善+サブスク限定機能+Discord対応+トリミング＋位置調整まで完了。実際に使った人の声を聞いてから次の判断をするのが最も効率的。
