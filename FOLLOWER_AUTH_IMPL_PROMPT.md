...
### Stage 8: killswitch 動作確認 + 統合テスト + fix8 連動解除

**目的**: 環境変数による緊急縮退が機能することを確認。code 修正は最小、主に手動テスト。さらに fix8（プライバシーポリシー実装）で fix7 リリースまで一時コメントアウトされていた PP §1-3 のキャッシュ情報行を有効化する。

**変更ファイル**:
- `.env.example` — 新環境変数 4 つ追記（コメント付き）
- `src/app/privacy/page.tsx` — **PP §1-3 のコメントアウト解除**（下記 fix8 連動タスク参照）
- 必要なら微修正・コメント追加のみ
...
**fix8 連動タスク（PP §1-3 コメントアウト解除）**:

fix8 で `src/app/privacy/page.tsx` の §1-3 にある「アクセス権限に関するキャッシュ情報」行が、fix7 リリースまで一時的に JSX コメントアウトされている。fix7 で 24h stale-cache 機構を実装した本ステージで**有効化**する必要がある。

- 対象ファイル: `src/app/privacy/page.tsx`
- 対象行: `{/* TODO(fix7): フォロワー認証実装後に有効化 */}` コメントが付いた `<li>アクセス権限に関するキャッシュ情報</li>`
- 解除方法:
  ```jsx
  // 解除前（fix8 commit 06db947 時点）:
  {/* TODO(fix7): フォロワー認証実装後に有効化 */}
  {/* <li>アクセス権限に関するキャッシュ情報</li> */}

  // 解除後（fix7 Stage 8 完了時の状態）:
  <li>アクセス権限に関するキャッシュ情報</li>
  ```
  → JSX コメント `{/* ... */}` を 2 個取り除いて `<li>` 要素を有効化するだけ。
- PP 最終更新日（`src/app/privacy/page.tsx` の冒頭の `最終更新日：YYYY年M月D日`）も合わせて fix7 リリース日に更新する。

**fix8 連動タスク（PP §1-1 注釈削除）**:

fix8 の `src/app/privacy/page.tsx` の §1-1「Twitch アカウント連携時に取得する情報」リストには、フォロー判定機能が将来実装予定であることを示す `*` 注釈が 2 箇所付いている。fix7 で実装が完了した本ステージで**注釈を消し、通常の取得情報として扱う**必要がある。

- 対象ファイル: `src/app/privacy/page.tsx`
- 対象行（§1-1 内、fix8 commit 06db947 時点）:
  ```jsx
  <li>
  ...
  ```
- 解除方法:
  - 各 `<li>` 内の `<span className="text-gray-400"> *</span>` を削除
  - その下の `<p>...将来的に実装予定です...</p>` ブロック全体を削除
- 解除後の状態:
  ```jsx
  <li>
    @datsusara_aki チャンネルへのフォロー有無
  ...
  ```
  注釈 `<p>` ブロックは丸ごと削除。
- PP 最終更新日も上記 §1-3 解除と同タイミングで fix7 リリース日に更新する（重複作業にならないよう同じコミット内で実施）。
...
[ ] PP §1-3 のコメントアウトを解除する
      対象ファイル: src/app/privacy/page.tsx
      対象行: TODO(fix7) コメントが付いた `<li>アクセス権限に関するキャッシュ情報</li>`
      解除方法: JSX コメント `{/* ... */}` を取り除いて li 要素を有効化
      これは fix8（プライバシーポリシー実装）で fix7 リリースまで一時的に
      コメントアウトされていた行で、フォロワー認証実装後に有効化する必要がある
[ ] PP §1-1 の `*` 注釈削除
      対象ファイル: src/app/privacy/page.tsx
      対象行: §1-1 の「@datsusara_aki チャンネルへのフォロー有無 *」と
        「上記チャンネルへのフォロー日時（フォローしている場合）*」
      解除方法: 末尾の ` *`（`<span className="text-gray-400"> *</span>`）を取り除く
        + 注釈テキスト「* フォロー判定機能は将来的に実装予定です…」のブロック全体を削除
      これは fix8（プライバシーポリシー実装）で fix7 リリースまで
      「将来実装予定」として記載した内容で、フォロー判定実装後に
      通常の取得情報として扱う必要がある
[ ] PP 最終更新日 (src/app/privacy/page.tsx 冒頭) を fix7 リリース日に更新済み
```
